...
def main() -> int:
    ...
    p.add_argument("--preds", nargs="+", default=None,
                   help="test-prediction npz files, `path` or `label=path` (default: ts_only_<index>_test_preds.npz)")
    args = p.parse_args()
    cfg = yaml.safe_load(Path(args.config).read_text())
    idx = cfg["index"]
    d = default_data_dir()

    # each entry is a path or `label=path`; with a label every key of that file is prefixed `label_`
    specs = args.preds or [str(d / f"ts_only_{idx}_test_preds.npz")]
    long, models = [], []
    for i, spec in enumerate(specs):
        label, _, f = spec.rpartition("=")
        z = np.load(Path(f).expanduser())
        dates = pd.to_datetime(z["dates"])
        for k in (["y"] if i == 0 else []) + [k for k in z.files if k not in ("dates", "y")]:
            name = k if (k == "y" or not label) else f"{label}_{k}"
            if name in models:  # a reference already loaded from an earlier file is skipped
                continue
            df = pd.DataFrame(z[k][:, : args.max_lead], index=dates, columns=range(1, args.max_lead + 1))
            long.append(df.stack().rename(name))
            if name != "y":
                models.append(name)
